...
## Sprite Selection
Characters should look like what they are. Instead of a random glyph from the Kenney sheet, each character picks a sprite from a tagged 490-sprite library (godot/assets/players_v2.png, CC0 DCSS-derived) whose traits match the genome — heavy brawlers look heavy, zoners look like casters, and a deliberate slice of the roster stays goofy. Sprite identity is a heritable gene (children inherit a parent's look, repaired only when it stops making sense), selection shares its seed and register with the name generator so a fighter's look and name never disagree, and the whole pass is deterministic from (genome, seed). The full implementation brief lives at docs/features/sprite-selection.md; the library is regenerated by tools/sprite-pipeline/ (see its README) against docs/art/style-spec.md, with sources audited in docs/art/asset-manifest.md.
